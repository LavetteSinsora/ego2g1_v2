"""
python test/test_replay.py --repo-id unitreerobotics/G1_CameraPackaging_NewDataset --robot_type unitree_g1_dex1
python test/test_replay.py --repo-id unitreerobotics/Z1_StackBox_Dataset --robot_type z1_realsense
python test/test_replay.py --repo-id unitreerobotics/Z1_Dual_Dex1_StackBox_Dataset_V2 --robot_type unitree_z1_dual_dex1_realsense
"""
...
# Replay a specific episode from the LeRobot dataset using the real environment  robot_type：(e.g., unitree_g1_dex1, unitree_z1_realsense, unitree_z1_dual_dex1_realsense)
def replay_lerobot_data(repo_id: str, robot_type: str, root: str | None = None, episode: int = 0):
    ...
    try:
        ...
        if user_input.lower() == "s":
            ...
            for idx in range(dataset.num_frames):
                ...
                if robot_type == "unitree_g1_wbc_dex1":
                    action = np.concatenate([action[:14], action[-7:], action[14:16]])
                elif robot_type == "z1_realsense":
                    action[-1] = -action[-1]
