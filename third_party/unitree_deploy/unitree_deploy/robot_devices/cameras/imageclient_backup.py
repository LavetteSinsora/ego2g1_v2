...
from unitree_deploy.robot_devices.cameras import zmq_msg
...
class ImageClientCamera:
    ...
    def connect(self):
        try:
            ...
            self.img_client = ImageClient(self.host_ip)
    ...
    def async_read(self):
        try:
            ...
            if self.camera_config['head_camera']['enable_zmq']:
                head_img, head_img_fps = self.img_client.get_head_frame()
                if head_img is not None:
                    colors["cam_left_high"] = head_img[:, :self.camera_config['head_camera']['image_shape'][1]//2]
                    colors["cam_right_high"] = head_img[:, self.camera_config['head_camera']['image_shape'][1]//2:]

            if self.camera_config['left_wrist_camera']['enable_zmq']:
                colors["cam_left_wrist"], _ = self.img_client.get_left_wrist_frame()
            if self.camera_config['right_wrist_camera']['enable_zmq']:
                colors["cam_right_wrist"], _ = self.img_client.get_right_wrist_frame()
