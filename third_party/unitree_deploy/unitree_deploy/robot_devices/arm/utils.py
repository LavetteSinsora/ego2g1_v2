...
def make_arm_motors_bus(arm_type: str, **kwargs) -> Arm:
    ...
    elif arm_type == "g1_wbc":
        from unitree_deploy.robot_devices.arm.g1_wbc_arm import G1_WBC_ArmController

        config = G1WbcArmConfig(**kwargs)
        return G1_WBC_ArmController(config)
    else:
