...
from unitree_deploy.robot_devices.robot_devices_index import Z1_12_JointArmIndex
...
class Z1_12_ArmController:
    ...
    def connect(self):
        try:
            ...
            self.publish_thread = self._start_daemon_thread(
                self._ctrl_motor_state, name="z1_dual._ctrl_motor_state"
            )
    ...
    def _schedule_waypoint(
        ...
        self._update_z1_arm(
            ...
            tau=arm_tauff_target[: self.arm_indices_len]
            if arm_tauff_target is not None
            else arm_tauff_target,
        )
        self._update_z1_arm(
            arm=self.z1_right,
    ...
    def arm_fk(
        self, left_q: np.ndarray | None = None, right_q: np.ndarray | None = None
    ) -> np.ndarray | None:
