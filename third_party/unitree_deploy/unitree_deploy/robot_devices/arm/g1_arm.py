...
from unitree_deploy.robot_devices.robot_devices_index import G1_29_JointArmIndex, G1_29_JointIndex
...
class G1_29_ArmController:
    ...
    def _start_daemon_thread(
        self, target_fn: Callable[[], None], name: str | None = None
    ) -> threading.Thread:
    ...
    def connect(self):
        try:
            ...
            self.publish_thread = self._start_daemon_thread(
                self._ctrl_motor_state, name="g1._ctrl_motor_state"
            )
    ...
    def _subscribe_motor_state(self):
        try:
            while not self.stop_event.is_set():
                ...
                if self.mock:
                    if (
                        self.g1.get_current_positions() is not None
                        and len(self.g1.get_current_positions()) != 0
                    ):
    ...
    def _drive_to_waypoint(self, target_pose: np.ndarray, t_insert_time: float):
        ...
        while time.monotonic() < t_insert:
            ...
            self._update_g1_arm(clipped_arm_q_target, self.dq_target, self.tauff_target)
    ...
    def _schedule_waypoint(
        ...
        self._update_g1_arm(clipped_arm_q_target, self.dq_target, arm_tauff_target)
    ...
    def arm_ik(
        ...
        return self.g1_arm_ik.solve_ik(
            l_ee_target, r_ee_target, self.read_current_arm_q(), self.read_current_arm_dq()
        )
