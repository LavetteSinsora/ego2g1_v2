...
from unitree_deploy.robot_devices.robot_devices_index import Z1GripperArmJointIndex
...
class Z1ArmController:
    ...
    def connect(self):
        try:
            ...
            self.publish_thread = self._start_daemon_thread(
                self._ctrl_motor_state, name="z1._ctrl_motor_state"
            )
    ...
    def read_current_arm_q_without_gripper(self) -> np.ndarray:
        """Return current state q of the left and right arm motors."""
        return np.array(
            [self.lowstate_buffer.get_data().motor_state[id].q for id in list(Z1GripperArmJointIndex)[:-1]]
        )

    def read_current_gripper_q(self) -> np.ndarray:
        """Return current state q of the left and right arm motors."""
        return np.array(
            [self.lowstate_buffer.get_data().motor_state[list(Z1GripperArmJointIndex)[-1].value].q]
        )
