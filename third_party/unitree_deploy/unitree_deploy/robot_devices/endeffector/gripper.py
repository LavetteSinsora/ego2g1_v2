...
from unitree_deploy.robot_devices.robot_devices_index import Gripper_Single_JointIndex
...
class Dex1_Gripper_Controller:
    def __init__(self, config: Dex1GripperConfig):
    ...
    def connect(self):
        try:
            ...
            self.gripper_msg.cmds = [
                unitree_go_msg_dds__MotorCmd_() for _ in range(len(Gripper_Single_JointIndex))
            ]
            for id in Gripper_Single_JointIndex:
        ...
        try:
            while True:
                ...
                if gripper_msg is not None:
                    ...
                    for idx, id in enumerate(Gripper_Single_JointIndex):
    ...
    def _update_gripper(self, gripper_q_target: np.ndarray):
        current_qs = np.array(
            [self.lowstate_buffer.get_data().motor_state[id].q for id in Gripper_Single_JointIndex]
        )
        clamped_qs = np.clip(
            gripper_q_target, current_qs - self.DELTA_GRIPPER_CMD, current_qs + self.DELTA_GRIPPER_CMD
        )
    ...
    def read_current_endeffector_dq(self) -> np.ndarray:
        ...
        motor_states_dq = np.array(
            [self.lowstate_buffer.get_data().motor_state[id].dq for id in Gripper_Single_JointIndex]
        )
