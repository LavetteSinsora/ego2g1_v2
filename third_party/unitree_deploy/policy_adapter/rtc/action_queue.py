...
class ActionQueue:
    ...
    def _check_and_resolve_delays(
        ...
        if action_index_before_inference is not None:
            ...
            # ⚠️ IMPROVED: Don't warn if mismatch is due to slow execution
            # This can happen if the robot execution is slower than expected
            # In such cases, we use the actual consumption (indexes_diff) as the reference
            if indexes_diff != real_delay:
                # Only warn if the deviation is extreme (> 100% difference)
                deviation_percent = abs(real_delay - indexes_diff) / max(1, real_delay) * 100

                if deviation_percent > 100:
                    logger.warning(
                        "Large delay mismatch detected. indexes_diff=%d, real_delay=%d (deviation=%.0f%%)",
                        indexes_diff,
                        real_delay,
                        deviation_percent,
                    )
                else:
                    # Minor deviation: use the actual consumption as ground truth
                    # This adapts RTC to the system's actual execution capability
                    effective_delay = max(1, indexes_diff)
                    if indexes_diff > 0:
                        logger.debug(
                            "Adapting delay: using actual consumption %d instead of computed %d",
                            indexes_diff,
                            real_delay,
                        )
