...
# The model-space row guards moved to actions.py (next to the layouts they
# check, and now actually CALLED per row by the converters — before the
# refactor sanity_check_model_action was defined here and called by nobody,
# and the relation_eef mode had no guard at all). Re-exported for callers
# and tests that know them by this module's name.
sanity_check_model_action = _actions.sanity_check_model_action
sanity_check_relation_action = _actions.sanity_check_relation_action
