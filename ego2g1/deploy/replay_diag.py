...
def main(args: Args) -> None:
    ...
    try:
        row[_actions.ARM] = arm[0]
        for h in layout.HANDS:
            row[_actions.HAND[h]] = ep["hand"][h][0] if args.hands else 0.0
        executor.send(row)          # vendor drive_to_waypoint soft ramp
        if not args.dry_run:
            time.sleep(2.0)
        ...
        for k in range(n):
            ...
            executor.send(row, t0 + (k + 2) * period)   # future-stamped
            t_log[k] = t
            qc_log[k] = arm[k]
            qm_log[k] = executor.arm_q()
            precise_wait(t0 + (k + 1) * period)
