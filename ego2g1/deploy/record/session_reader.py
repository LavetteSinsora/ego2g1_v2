...
class Session:
    def __init__(self, session_dir):
        ...
        # v1 == recorded before record/schema.py existed (no key). Readers
        # keep the v1 fallbacks (e.g. replay_mujoco's commanded-joints-only
        # rendering) — a missing key is a version statement, not an error.
        ...
        unknown = {e["kind"] for e in self.events} - set(_schema.EVENT_KINDS)
        if unknown:
            logger.warning(
                "%s: unknown event kind(s) %s — recorded by a newer schema? "
                "(this session: v%d, this reader: v%d). Ignoring them.",
                self.dir.name, sorted(unknown), self.schema_version,
        ...
        self._percepts = self._by_kind.get("percept", [])
        self._percept_t = [e["t"] for e in self._percepts]
    ...
    def _make_buffer(self):
        ...
        missing = [k for k in _schema.STRATEGY_PARAM_KEYS if k not in self.meta]
        if missing:
            logger.warning(
                "%s: meta.json is missing strategy param(s) %s — falling back "
                "to defaults. The reconstruction below may not match what ran "
                "live (recordings made via record.schema.build_meta always "
                "carry them).", self.dir.name, missing)
        if self.mode == "async":
    ...
    def percept_at(self, t: float) -> dict | None:
        """The last `percept` event (`RelationPerception.debug_snapshot()`'s
        payload, logged by `runner.py`'s relation_eef drain step) at or
        before `t` -- `{"objects": {...}, "hands": {...}}`, or `None` if this
        session predates `percept` events (older recordings) or isn't a
        relation_eef session at all."""
        i = bisect.bisect_right(self._percept_t, t) - 1
        if i < 0:
            return None
