...
class PreviewLoop:
    ...
    def telemetry(self) -> dict:
        ...
        rel = None
        if self.adapter.last_percept is not None:
            rel = relation_panel(
                self._perception.debug_snapshot(include_masks=False),
                self._perception.recent_events())
        return TelemetrySnapshot(
            now=time.monotonic(), mode="perception-preview",
            active=True,
            task="perception preview -- real detector, NO robot/policy",
            horizon=0, fps=0, dim=0, groups=[],
...
def main() -> None:
    ...
    p.add_argument("--camera-host", default=_camera.DEFAULT_HOST,
