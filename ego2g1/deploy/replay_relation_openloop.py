...
def main(args: Args) -> None:
    ...
    clamp = _safety.Clamp(_safety.SafetyLimits(max_joint_step=args.max_step))
    clamp.reset(executor.arm_q())

    stamp = pathlib.Path(args.record_dir) / f"relation_openloop_{ep['name']}_{int(time.time())}"
    rec = _recorder.Recorder(stamp, meta={
    ...
    try:
        ...
        while t0 < ep["length"]:
            ...
            for k in range(len(joints)):
                ...
                row = joints[k].copy()
                row[_actions.ARM] = clamp(row[_actions.ARM], dt)
                executor.send(row, t_cycle_end + dt)
                rec.log("action", step=n_chunks, row=row)
